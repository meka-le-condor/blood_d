...
import HeaderDonor from './HeaderDonor'
import axios from 'axios';

function Listeorga()  {
    const  [datad,setData]=useState([]);
    axios.get('http://localhost:4002/api/get/orga')
        .then(reponse=>{
...
<div class="relative flex flex-col items-center p-4 sm:max-w-xl">
 ...
 <h1 class="mb-8 text-center text-4xl font-bold text-white sm:text-5xl md:mb-12 md:text-6xl">Liste des Organisations disponibles pour un don de sang</h1>
     ...
     <div class="flex w-full flex-col gap-2.5 sm:flex-row sm:justify-center">
         ...
         datad.map(function(item){
             return(
                 ...
                 <div class="my-4">
                     <h2 class="text-white text-2xl font-bold pb-2">{item.orgaName}</h2>
                     <p class="text-gray-300 py-1">Adresse : {item.address}</p>
                 </div>

                 <div class="flex justify-end">
                     <button class="px-2 py-1 text-white border border-gray-200 font-semibold rounded hover:bg-gray-800">RV</button>
                 </div>
             </div>
